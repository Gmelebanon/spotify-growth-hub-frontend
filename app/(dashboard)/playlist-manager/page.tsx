...
export default function PlaylistManagerPage() {
  ...
  const updateSelectedCurationTracks = (
    tracks: AddedTrack[],
    options: { keepSelection?: boolean; saveUndo?: boolean } = {},
  ) => {
    if (!selectedCurationBox) return;

    if (options.saveUndo !== false) {
      setTrackUndoStack((current) => [
        selectedCurationBox.tracks,
        ...current,
      ].slice(0, 20));
    }
    ...
    if (!options.keepSelection) {
      ...
      setSelectedTrackIndexes((current) =>
        current.filter((index) => index >= 0 && index < tracks.length),
      );
    }
  ...
  const handleUndoTrackChange = () => {
    if (!selectedCurationBox || trackUndoStack.length === 0) return;

    const [previousTracks, ...remaining] = trackUndoStack;
    setTrackUndoStack(remaining);
    setSelectedTrackIndexes([]);
    setLastSelectedTrackIndex(null);
    ...
    persistState({
      ...state,
      masterCurationBoxes: state.masterCurationBoxes.map((box) =>
        box.id === selectedCurationBox.id ? { ...box, tracks: previousTracks } : box,
      ),
    });
    setPageMessage("Last track change undone.");
  };

  const handleSelectCurationTrack = (trackIndex: number, shiftKey = false) => {
    if (!selectedCurationBox) return;
    ...
    setSelectedTrackIndexes((current) => {
      ...
      if (current.includes(trackIndex)) {
        return current.filter((index) => index !== trackIndex);
      }

      return [...current, trackIndex].sort((a, b) => a - b);
    });
  ...
  const handleSelectAllCurationTracks = () => {
    if (!selectedCurationBox) return;
    setSelectedTrackIndexes(selectedCurationBox.tracks.map((_, index) => index));
    setLastSelectedTrackIndex(
      selectedCurationBox.tracks.length > 0 ? selectedCurationBox.tracks.length - 1 : null,
    );
  };
  ...
  const handleDeleteSelectedTracks = () => {
    if (!selectedCurationBox || selectedTrackIndexes.length === 0) return;

    const selected = new Set(selectedTrackIndexes);
    const tracks = selectedCurationBox.tracks.filter((_, index) => !selected.has(index));
    updateSelectedCurationTracks(tracks);
    setPageMessage(`${selectedTrackIndexes.length} selected track${selectedTrackIndexes.length === 1 ? "" : "s"} deleted.`);
  };
  ...
  useEffect(() => {
    ...
    setLastSelectedTrackIndex(null);
    setTrackUndoStack([]);
  }, [state.selectedSavedMasterPlaylistId, selectedCurationBox?.id]);
            ...
            <div className="mt-6 rounded-2xl border border-zinc-800 p-4">
              <div className="mb-4 flex items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    disabled={!selectedCurationBox || trackUndoStack.length === 0}
                    onClick={handleUndoTrackChange}
                    title="Undo last track change"
                    aria-label="Undo last track change"
                    className="flex h-12 w-12 items-center justify-center rounded-xl border border-zinc-700 text-white hover:bg-zinc-900 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    <svg
                      aria-hidden="true"
                      viewBox="0 0 64 64"
                      ...
                      fill="currentColor"
                    >
                      <path d="M26.5 12.5 5.5 31.5l21 19v-12h13.2c8.3 0 14.6 3.4 18.8 10.1-1.8-15.9-10.2-24.1-25.2-24.1h-6.8v-12Z" />
                    </svg>
                  </button>
              ...
              {!selectedCurationBox || selectedCurationBox.tracks.length === 0 ? (
                ...
                <div className="scrollbar-spotify max-h-[520px] space-y-2 overflow-y-auto pr-2">
                  {selectedCurationBox.tracks.map((track, index) => {
                    const isSelected = selectedTrackIndexes.includes(index);
                    ...
                    return (
                      <div
                        ...
                        onClick={(event) => handleSelectCurationTrack(index, event.shiftKey)}
                        className={`cursor-grab rounded-xl border px-4 py-3 active:cursor-grabbing ${
                          isSelected
                            ? "border-green-400 bg-green-500/10"
                            : "border-green-500/40 bg-zinc-950"
                        }`}
                      >
                          ...
                          <div className="flex min-w-0 items-center gap-3">
                            <input
                              ...
                              onClick={(event) => {
                                ...
                                handleSelectCurationTrack(index, event.shiftKey);
                              }}
                              onChange={() => undefined}
                              className="h-4 w-4"
                            />
                      ...
                      <div className="min-w-0">
                        ...
                        allPlaylists.find(
                          (item) =>
                            ...
                            item.accountId === playlist.accountId,
                        )?.spotify_url ? (
                          <a
                            href={
                              playlist.spotifyUrl ||
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="block truncate text-sm font-semibold text-white hover:text-green-400 hover:underline"
                          >
                            {playlist.name}
                          </a>
                        ) : (
